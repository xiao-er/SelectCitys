...
/**
 * file name文件名：InternatCityFragment
 * Description 描述：
 * Created time 创建时间：2017/12/7 14:33
 * author 作者：潇潇
 * email：15010222898@163.com
 */
public class InternatCityFragment extends Fragment {
    @BindView(R.id.rv)
    ...
    Unbinder unbinder;
    private List<CityListBean> internetCityList = new ArrayList<CityListBean>();
    ...
    private void initData() {
        internetCityList = (List<CityListBean>) getArguments().getSerializable("internatCityList");
    }
    ...
    private void initView() {
        ...
        mAdapter = new CommonAdapter<CityListBean>(getActivity(), R.layout.citylist_item_select_city, mBodyDatas) {
            ...
            public void convert(ViewHolder holder, final CityListBean cityBean) {//主体城市显示的Adapter
                //当城市简称和城市对应国家都不为空的时候
                if ((cityBean.getCityShortCode() != null && !cityBean.getCityShortCode().isEmpty()) && (cityBean.getCityCounty() != null && !cityBean.getCityCounty().isEmpty())) {
                    holder.setText(R.id.txt_cityOrhterInfo, cityBean.getCityShortCode() + "   " + cityBean.getCityCounty());
                }
                //如果城市简称为空  城市对应国家不为空
                else if ((cityBean.getCityShortCode() == null || cityBean.getCityShortCode().isEmpty()) && (cityBean.getCityCounty() != null && !cityBean.getCityCounty().isEmpty())) {
                    holder.setText(R.id.txt_cityOrhterInfo, cityBean.getCityCounty());
                }
                //如果城市简称不为空，城市对应国家为空
                else if ((cityBean.getCityShortCode() != null && !cityBean.getCityShortCode().isEmpty()) && (cityBean.getCityCounty() == null || cityBean.getCityCounty().isEmpty())) {
                    holder.setText(R.id.txt_cityOrhterInfo, cityBean.getCityShortCode());
                } else {
        ...
        CityListBean[] cityDatasNew = new CityListBean[internetCityList.size()];
        for (int i = 0; i < internetCityList.size(); i++) {
            CityListBean mCityListBean = new CityListBean();
            mCityListBean.setCity(internetCityList.get(i).getCity());
            mCityListBean.setInitials(internetCityList.get(i).getInitials());
            mCityListBean.setCityShortCode(internetCityList.get(i).getCityShortCode());
            mCityListBean.setCityCounty(internetCityList.get(i).getCityCounty());
        ...
        getActivity().getWindow().getDecorView().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i < data.length; i++) {
                    ...
                    cityBean.setCityCounty(data[i].getCityCounty());
